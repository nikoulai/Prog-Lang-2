...
main =
  do all <- BS.getContents
     ...
     -- let stringMap = convertToMap (Map.empty) (n-1) ( drop 1 $ BSC.unpack r1 )
     ...
     -- print stringMap
     -- print mapKeyLetter
     -- let result = countPalindromes n stringMap
     print $ (Maybe.fromMaybe  0 $ result) `mod` 20130401
     -- print result
     -- let i =2
...
countPalindromes n stringArray mapKeyLetter =  Map.lookup (0,(n-1)) dp
 where
  d i j | i >= n || j < 0 = 0
       ...
       | abs(i-j) == 1 = if List.elem j (Maybe.fromMaybe  [1,2] $ Map.lookup (Maybe.fromMaybe  'f' $ Map.lookup i stringArray) mapKeyLetter) then 3 else 2
       | i == j =  1
       | List.elem j (Maybe.fromMaybe  [1,2] $ Map.lookup (Maybe.fromMaybe  'f' $ Map.lookup i stringArray) mapKeyLetter) = (Maybe.fromMaybe  0 $ Map.lookup (i+1,j) dp)+ (Maybe.fromMaybe 0 $ Map.lookup (i, j-1) dp) +1
       | otherwise = (Maybe.fromMaybe  0 $ Map.lookup (i+1,j) dp) + (Maybe.fromMaybe  0 $ Map.lookup (i, j-1) dp) - (Maybe.fromMaybe  0 $ Map.lookup (i+1, j-1) dp)
       -- | otherwise = 2

  dp = Map.fromList [((l,i), d l i ) | (l,i) <- Array.range  ((0,0),(n+1,n+1)) ]
